...
treinadoras = {treinadora1.id: treinadora1,treinadora2.id: treinadora2, treinadora3.id: treinadora3}
...
#renderizando template lista e as variaveis desejadas
...
@app.route('/login')
def login():
    proxima = request.args.get('poxima')
    return render_template('login.html', proxima=proxima)

#configuração da rota autenticar que verifica as credenciais das treinadoras

@app.route('/autenticar', methods=['POST',])
def autenticar():
    if request.form['treinadora'] in treinadoras:
        treinadoras = treinadoras[request.form['treinadora']]
